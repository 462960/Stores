  ...
  axios.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n           <div class="popup">\n            <a href="/store/' + this.place.slug + '">\n             <img src="/uploads/' + (this.place.photo || 'bb7f7d60-9b0c-46b0-a0e4-1422d5752567.jpeg') + '" alt="' + this.place.name + '" />\n             <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n            </a>\n           </div>\n           ';
        infoWindow.setContent(html);
        infoWindow.open(map, this);
      });
    });
    // Zoom the markers in to fit the page widow
...
function searchResultsHTML(search) {
	return search.map(function (store) {
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		axios.get('/api/search/?q=' + this.value).then(function (res) {
			...
			searchResults.innerHTML = dompurify.sanitize('<div class="search__result">Nothing for ' + _this.value + ' found</div>');
		}).catch(function (err) {
			console.error(err);
		});
	});
